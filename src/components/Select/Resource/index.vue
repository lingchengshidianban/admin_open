  ...
  <el-dialog :model-value="props.visible" :title="props.title" :destroy-on-close="true" :append-to-body="true" center align-center :width="1000" @close="onClose">
    ...
    <el-form :model="query" inline>
      ...
      <el-form-item>
        ...
        <UploadFile v-permission="'resource:edit'" :refresh="handleQuery" :on-bus="props.visible" :category-id="query.categoryId" />
      </el-form-item>
    </el-form>
    <div class="table-container">
      ...
      <div v-if="props.resourceType === 4" v-loading="page.loading" class="table-layout">
        <div v-for="(item, index) in page.list" :key="index" class="table-item" @click="handleCard(item)">
          ...
          <div v-if="props.resourceType === 4" class="table-title">
          ...
          <template #default="scope">
            <span>
              ...
              <span v-if="scope.row.resourceType > 1">
                【
                <EnumView :enum-value="scope.row.storagePlatform" :enum-name="'StoragePlatformEnum'" />
                】
              </span>
              <span v-if="scope.row.resourceType < 3">
                【
                <EnumView :enum-value="scope.row.vodPlatform" :enum-name="'VodPlatformEnum'" />
                】
              </span>
              <br />
              <span>
